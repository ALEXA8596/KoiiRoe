...
const { getTokensByOwner, getTokenList } = require('./koii-token-list-service');
const {
  SOL_DECIMALS,
  SOL_SYMBOL,
  SOL_NAME,
...
const { KOII } = require('../../constants/platforms');
 ...
 * @returns 
 */
const getKoiiBalance = async (connection, publicKey) => {
...
const getPrices = async () => {
  try {
    return await getPricesByPlatform(KOII);
  } catch (e) {
...
const getBalance = async (connection, publicKey) => {
  ...
  const koiiBalance = await getKoiiBalance(connection, publicKey);
  const prices = await getPrices();
  const balances = await decorateBalancePrices([koiiBalance, ...tokensBalance], prices);
  if (prices) {

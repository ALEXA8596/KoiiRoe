...
} = require('./koii-token-service');
const { getTokenByAddress } = require('./koii-token-list-service');
const { applyDecimals } = require('./koii-token-service');
...
const createTransaction = async (
  ...
  if (token == SOL_ADDRESS) {
    transaction = await transactionKoii(connection, fromKeyPair, toPublicKey, amount);
  } else {
    transaction = await transactionKpl(connection, fromKeyPair, toPublicKey, token, amount, opts);
  }
  const result = await execute(connection, transaction, fromKeyPair, simulate);
  return { txId: result };
};

const transactionKoii = async (connection, fromKeyPair, toPublicKey, amount, opts = {}) => {
  const { simulate } = opts;
...
const estimateFee = async (connection, fromKeyPair, toPublicKey, token, amount) => {
  ...
  if (token == SOL_ADDRESS) {
    ...
    transaction = await transactionKpl(connection, fromKeyPair, toPublicKey, token, amount);
  }
